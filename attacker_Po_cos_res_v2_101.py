...
tf.flags.DEFINE_string('loss', 'cepo', 'ce: cross_entry, po: poincare, cepo: ce+po')

tf.flags.DEFINE_string('cuda', '3', 'which cuda device can use')

tf.flags.DEFINE_float('a', 0.007, 'alpha of the loss of cosine, threshold')

tf.flags.DEFINE_float('W_cos', 0.01, 'weight of ')

tf.flags.DEFINE_string('logname', 'res_v2_101', 'which model')
...
def graph(x, y, i, x_max, x_min, grad, y_target, y_logits):
    ...
    with slim.arg_scope(resnet_v2.resnet_arg_scope()):
        logits_resnet_101, end_points_resnet_50 = resnet_v2.resnet_v2_50(
            input_diversity(x), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)

    # with slim.arg_scope(resnet_v2.resnet_arg_scope()):
    #     logits_resnet_50, end_points_resnet_101 = resnet_v2.resnet_v2_101(
    #         input_diversity(x),  num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)


    logits = (logits_resnet +
              logits_v3 + logits_resnet_101 +
              logits_res_v2 + logits_v4 ) / 5
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with open('time_' + FLAGS.logname, 'w') as f:
